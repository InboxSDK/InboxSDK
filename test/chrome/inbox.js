  ...
  it('works', function() {
    try {
      ...
      browser.waitForVisible(
        'div[role=dialog] div[jsaction^=compose]',
        10 * 1000
      );
      ...
      );
      {
        ...
        subject.click();
        subject.keys(
          `InboxSDK Inbox ComposeView events test @ ${new Date().getTime()}`
        );
        body.click();
...

      {
        ...
        subject.click();
        subject.keys(
          `InboxSDK Inbox ComposeView cancel send test @ ${new Date().getTime()}`
        );
        body.click();
        body.keys('Test message!');
        browser.click('div[role=dialog] [jsaction$=send]');
        browser.waitUntil(
          () =>
            Number(
              headEl.getAttribute('data-test-composePresendingEmitted')
            ) === 2,
          10 * 1000
        );
        assert(
          browser.isVisible('div[role=dialog] div[jsaction^=compose]', 1000)
        );
        browser.click('button[jsaction^=compose][jsaction$=discard_draft]');
        assert.strictEqual(
          Number(headEl.getAttribute('data-test-composePresendingEmitted')),
      ...
      browser.click(
        '.scroll-list-section-body div[role=listitem][data-item-id-qs*="thread-"] span[email="no-reply@accounts.google.com"]'
      );
      // The thread might take some time to load
      browser.waitForVisible(
        'div[jsaction*=quickCompose][jsaction$=quick_compose_handle_focus]',
        30 * 1000
      );
      browser.pause(500);
      browser.click(
      ...
      assert.strictEqual(
        browser.getText('.test__sidebarCounterButton'),
        'Counter: 0'
      );
      ...
      assert.strictEqual(
        browser.getText('.test__appToolbarCounterButton'),
        'Counter: 0'
      );
      ...
      const switchToOverlayFrame = () => {
        ...
        for (let i = 0; i < frames.length; i++) {
          ...
          const el = browser.execute(() =>
            Boolean(document.querySelector('body > div[role=dialog][tabindex]'))
          ).value;
          if (el) return;
        }
      ...
      browser.scroll(
        '.scroll-list-section-body div[role=listitem][data-item-id-qs*="thread-"] span[email="inboxsdktest@gmail.com"]',
        0,
        -500
      );
      browser.click(
      ...
      const messageViewsWithNativeCardsSeen = browser.execute(() =>
        Number(
          (document.head: any).getAttribute(
            'data-test-messageViewsWithNativeCardsSeen'
          )
        )
      ).value;
      ...
      const searchSugggestionsClicked1 = browser.execute(() =>
        Number(
          (document.head: any).getAttribute(
            'data-test-searchSuggestionsClicked1'
          )
        )
      ).value;
      const searchSugggestionsClicked2 = browser.execute(() =>
        Number(
          (document.head: any).getAttribute(
            'data-test-searchSuggestionsClicked2'
          )
        )
      ).value;
      ...
      assert.strictEqual(
        resultsList.$$('li.inboxsdk__search_suggestion').length,
        3
      );
      assert.strictEqual(
        resultsList.$$('div.inboxsdk__search_suggestion_group').length,
        2
      );
      ...
      for (let i = 0; i < 50; i++) {
        ...
        if (
          resultsList.$$('li.inboxsdk__search_suggestion.inboxsdk__selected')
            .length === 1
        ) {
      ...
      browser.waitUntil(() => !resultsList.isVisible(), 5000);
...

      {
        ...
        const threadViewsWithIDsLookedUp = browser.execute(() =>
          Number(
            (document.head: any).getAttribute(
              'data-test-threadView-getThreadIDAsync'
            )
          )
        ).value;
        ...
        const messageViewsSeen = browser.execute(() =>
          Number(
            (document.head: any).getAttribute('data-test-messageViewsSeen')
          )
        ).value;
        ...
        const messageViewsWithIDsLookedUp = browser.execute(() =>
          Number(
            (document.head: any).getAttribute(
              'data-test-messageView-getMessageIDAsync'
            )
          )
        ).value;
