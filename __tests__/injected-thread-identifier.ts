...
test('threadIdentifier works', () => {
  ...
  const amb: HTMLElement = document.getElementById(':3g')!;
  ...
  const amb2: HTMLElement = document.getElementById(':35')!;
  ...
  amb.addEventListener('click', function (event: MouseEvent) {
    ...
    setTimeout(function () {
      ...
      Object.keys(requiredProps).forEach((key) => {
        expect((event as any)[key]).toBe((requiredProps as any)[key]);
      });
  ...
  const twi1: HTMLElement = document.getElementById(':42')!;
  ...
  const twi2: HTMLElement = document.getElementById(':42-dup')!;
