...
export default function getComposeViewDriverLiveSet(
  driver: InboxDriver,
  ...
  const inlineCompose = toItemWithLifetimeStream(
    tree.getAllByTag('inlineCompose')
  ).map(denodeify);
  const regularCompose = toItemWithLifetimeStream(
    tree.getAllByTag('regularCompose')
  ).map(denodeify);
  const fullscreenCompose = toItemWithLifetimeStream(
    tree.getAllByTag('fullscreenCompose')
  ).map(denodeify);
        ...
        error(err) {
          ...
          function getStatus() {
            return {
              ...
              regularLength: document.querySelectorAll(
                'body > div[id][jsaction] > div[id][class]:not([role]) > div[class] > div[id]'
              ).length,
              noJsActionLength: document.querySelectorAll(
                'body > div[id] > div[id][class]:not([role]) > div[class] > div[id]'
              ).length,
              noNotLength: document.querySelectorAll(
                'body > div[id][jsaction] > div[id][class] > div[class] > div[id]'
              ).length,
              noBodyDirectChildLength: document.querySelectorAll(
                'body div[id][jsaction] > div[id][class]:not([role]) > div[class] > div[id]'
              ).length,
              noBodyLength: document.querySelectorAll(
                'div[id][jsaction] > div[id][class]:not([role]) > div[class] > div[id]'
              ).length,
              // We can use class names for logging heuristics. Don't want to use
              // them anywhere else.
              classLength: document.querySelectorAll('div.ek div.md > div')
                .length,
              classEkLength: document.querySelectorAll('.ek').length,
              classMdLength: document.querySelectorAll('.md').length,
              composeHtml: Array.from(
