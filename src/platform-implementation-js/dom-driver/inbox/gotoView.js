...
export default function gotoView(
  driver: InboxDriver,
  routeID: string,
  params: ?RouteParams
): void {
  ...
  if (!driver.getCustomRouteIDs().has(routeID)) {
    ...
    if (!params) {
      // If params were given, then we don't assume it was a resolved/populated URL
    ...
    if (!startedWithHash) {
      driver
        .getLogger()
        .deprecationWarning('Router.goto resolved routeID without "#" prefix');
    }
  }
