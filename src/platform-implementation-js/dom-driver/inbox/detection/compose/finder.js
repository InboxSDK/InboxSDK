...
export default function finder(root: Document=document): Array<HTMLElement> {
  ...
  const inlineComposesByPopoutBtn =_.chain(root.querySelectorAll('div[role=main] div[role=list] ~ div div[jslog] div[jsvs] > button'))
    .filter(el => el.style.display !== 'none')
    .map(el => findParent(el, el => el.hasAttribute('jsvs')))
    .value();

  const inlineComposesByBody = _.chain(root.querySelectorAll('div[role=main] div[role=list] ~ div div[jsvs] div[role=textbox][contenteditable=true][tabindex="0"]'))
    .map(el => findParent(el, el => el.hasAttribute('jsvs')))
    .value();
