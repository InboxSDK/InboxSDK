...
function parser(el: HTMLElement) {
  ...
  const downloadButton: ?HTMLElement = ec.run('downloadButton', () => {
    const candidate: ?HTMLElement = t.toArray(
      Array.prototype.slice.call(
        el.querySelectorAll(
          'div[role=button][tabindex][data-tooltip]:not([aria-disabled=true]):not([aria-pressed]):not([aria-expanded])'
        )
      ),
      t.compose(
        t.filter(
        ...
        t.filter(el => {
          if (global.document) {
            ...
            return el.parentElement.classList.contains(
              'ndfHFb-c4YZDc-Wrql6b-C7uZwb-b0t70b'
            );
          }
        }),
        t.filter(el => {
          ...
          return (
            nextDivSibling &&
            nextDivSibling.getAttribute('aria-haspopup') === 'true'
          );
        }),
        t.take(1)
      )
    ...
    if (candidate.getAttribute('aria-label') === 'Print')
      throw new Error('Found print button instead');
    return candidate;
  });

  const buttonContainer: ?HTMLElement = downloadButton
    ? (downloadButton: any).parentElement
    : null;
