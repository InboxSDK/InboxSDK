...
function parser(el: HTMLElement) {
  ...
  const downloadButton: ?HTMLElement = ec.run('downloadButton', () => {
    const candidate: ?HTMLElement = t.toArray(Array.from(el.querySelectorAll('div[role=button][tabindex][data-tooltip]:not([aria-disabled=true]):not([aria-pressed]):not([aria-expanded])')), t.compose(
      t.filter(el =>
