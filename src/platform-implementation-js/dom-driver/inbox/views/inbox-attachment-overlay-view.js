...
class InboxAttachmentOverlayView {
  ...
  constructor(
    ...
    el: HTMLElement,
    parsed: Parsed,
    cardView: InboxAttachmentCardView
  ) {
    this._driver = driver;
    this._el = el;
    this._p = parsed;
  ...
  addButton(button: Object) {
    ...
    if (!buttonContainer)
      throw new Error('Missing buttonContainer, can not add button');
    ...
    if (downloadButton) {
      el.setAttribute(
        'data-tooltip-class',
        downloadButton.getAttribute('data-tooltip-class') || ''
      );
      el.setAttribute(
        'data-tooltip-align',
        downloadButton.getAttribute('data-tooltip-align') || ''
      );
    }
  ...
  async getDownloadURL(): Promise<?string> {
    ...
    const src = await this._driver
      .getPageCommunicator()
      .clickAndGetNewIframeSrc(downloadButton);
