...
class InboxAppToolbarButtonView {
  ...
  constructor(
    ...
    appToolbarLocationLiveSet: LiveSet<TagTreeNode<HTMLElement>>,
    searchBarLiveSet: LiveSet<TagTreeNode<HTMLElement>>
  ) {
    this._buttonDescriptorStream = buttonDescriptor
      .toProperty()
  ...
  _setupButton(appToolbarLocation: HTMLElement) {
    ...
    if (!appToolbarButtonContainer) {
      ...
      appToolbarLocation.insertBefore(
        appToolbarButtonContainer,
        appToolbarLocation.firstChild
      );
    }
    ...
    Kefir.merge([
      ...
      .onValue(event => {
        ...
        if (this._activeDropdown) {
          ...
          if (
            !this._buttonDescriptor ||
            this._buttonDescriptor.hasDropdown !== false
          ) {
            this.open();
          } else {
    ...
    appToolbarButtonContainer.insertBefore(
      button,
      appToolbarButtonContainer.firstChild
    );
...
`;
  ...
  open() {
    ...
    const dropdown = (this._activeDropdown = new DropdownView(
      new InboxAppToolbarTooltipView(button, arrowColor),
      button
    ));
    ...
    return this._ready
      ...
      .merge(
        this._stopper.flatMap(() =>
          Kefir.constantError(
            new Error('InboxAppToolbarButtonView was destroyed early')
          )
        )
      )
      .take(1)
