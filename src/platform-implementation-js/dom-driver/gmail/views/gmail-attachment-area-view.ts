...
class GmailAttachmentAreaView {
  ...
  getAttachmentCardViews() {
    const attachments =
      this._element.querySelectorAll<HTMLElement>('.aQH > span');
  ...
  _createAreaToolbarIfNeeded(): HTMLElement {
    const preexistingToolbar = this._element.querySelector<HTMLElement>('.ho');
    ...
    toolbar.innerHTML = `<span class="aVW"><span>${
      this.getAttachmentCardViews().length
    }</span> Attachments</span><div class="aZi J-J5-Ji"></div>`;
  ...
  addGmailAttachmentCardView(gmailAttachmentCardView: GmailAttachmentCardView) {
    ...
    if (!zone) {
      this._driver
        .getLogger()
        .error(new Error('Could not find attachment zone'));
