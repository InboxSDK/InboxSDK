...
export default class GmailTooltipView {
  ...
  _containBoundingBox(
    ...
    if (boundingBox[0].y < -1) {
      ...
      boundingBox[1].y > ((document.body: any): HTMLElement).clientHeight
    ) {
      boundingBox[0].y =
        ((document.body: any): HTMLElement).clientHeight - boundingBoxHeight;
      boundingBox[1].y = boundingBox[0].y + boundingBoxHeight;
    }
    ...
    if (boundingBox[0].x < -1) {
      ...
      boundingBox[1].x > ((document.body: any): HTMLElement).clientWidth
    ) {
      boundingBox[0].x =
        ((document.body: any): HTMLElement).clientWidth - boundingBoxWidth - 20;
      boundingBox[1].x = boundingBox[0].x + boundingBoxWidth;
    }
  ...
  _setArrowPosition(
    ...
    switch (type) {
      ...
      case 'left':
        position.top =
          (containedBoundingBox[0].y + containedBoundingBox[1].y) / 2 -
          ARROW_VERTICAL_HEIGHT / 2;
      ...
      case 'right':
        position.top =
          ...
          ARROW_VERTICAL_HEIGHT / 2;
        position.left =
          containedBoundingBox[0].x - ARROW_HORIZONTAL_WIDTH / 2 + 1;
        break;
      case 'bottom':
        position.top = containedBoundingBox[0].y - ARROW_HORIZONTAL_HEIGHT + 1;
        position.left =
          (containedBoundingBox[0].x + containedBoundingBox[1].x) / 2;
        break;
    }
