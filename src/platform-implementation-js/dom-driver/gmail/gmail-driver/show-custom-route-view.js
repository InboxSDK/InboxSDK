...
const showCustomRouteView = defn(module, function showCustomRouteView(
  gmailDriver: any,
  element: HTMLElement
) {
  ...
  for (let i = 0; i < selection.rangeCount; i++) {
    ...
    if (
      typeof (range: any).intersectsNode === 'function' &&
      (range: any).intersectsNode(contentSectionElement)
    ) {
...
function _getCustomViewContainerElement(
  ...
  let customViewContainerElement = contentSectionElement.querySelector(
