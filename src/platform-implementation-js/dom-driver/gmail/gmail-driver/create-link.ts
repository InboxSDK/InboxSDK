...
export default function createLink(
  ...
  try {
    ...
    processedRoute = parts
      .map(function (part) {
        ...
        if (
          params &&
          typeof (params as RouteParams)[colonParts[1]] !== 'undefined'
        ) {
          return colonParts[0] + (params as RouteParams)[colonParts[1]];
        }
  ...
  if (
    GmailRouteProcessor.isListRouteName(parts[0]) &&
    processedRoute.indexOf('p0') === processedRoute.length - 2
  ) {
