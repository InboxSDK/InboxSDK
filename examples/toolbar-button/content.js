...
InboxSDK.load(1, 'toolbar-example', {
  ...
  inboxSDK.Toolbars.registerToolbarButtonForThreadView({
    ...
    onClick: function (event) {
      event.dropdown.el.textContent =
        'Subject: ' +
        event.threadView.getSubject() +
        '\n' +
        'Messages: ' +
        event.threadView
          .getMessageViews()
          .map(function (messageView) {
            messageView.getRecipients();
            return messageView.getSender().name;
          })
          .join(', ');
    },
  ...
  inboxSDK.Toolbars.registerToolbarButtonForList({
    ...
    hideFor: function (routeView) {
      return (
        routeView.getRouteID() === inboxSDK.Router.NativeListRouteIDs.DRAFTS
      );
    },
    onClick: function (event) {
  ...
  var button = inboxSDK.Toolbars.addToolbarButtonForApp({
    iconUrl:
      'https://ssl.gstatic.com/s2/oz/images/notifications/spinner_32_4152eb447e3e756250b29a0b19b2bbf9.gif',
    title: 'App Monkey',
    arrowColor: 'green',
    onClick: function (event) {
